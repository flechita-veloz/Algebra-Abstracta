...
int main(){
	...
	if(band){
		cout<<"Ingrese la palabra que desea cifrar: ";cin>>palabra;
		cifradoAfin(a, b, palabra);
	}
	if(!band){
		if(euclidesExtendido(a,27)==1){
			cout<<"Ingrese la palabra que desea descifrar: ";cin>>palabra;
			descifradoAfin(a, b, palabra);
		}
		else{
			cout<<"\n.:ERROR:.\n";
			cout<<"El coeficiente a debe tener inversa mod 27\n";
		}
	}
	return 0;
}
...
void cifradoAfin(int a, int b, string palabra){
	...
	for(int i=0; i< palabra.size(); i++){
		...
		int pos = (a*(palabra[i]-'A'+compensar)+b);
		pos = mod(pos,27);
